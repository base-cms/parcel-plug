...
const image = `basecms/parcel-plug-${service}`;

const nrIds = {
  delivery: 250053003,
  'event-processor': 271777402,
  graphql: 250053062,
};
...
const deploy = async () => {
  log(`Deploying ${image}:${version} on Rancher v1`);
  const nrid = nrIds[service];
  const { status } = await spawnSync('bash', ['scripts/deploy-rancher.sh', service, version, nrid], { stdio: 'inherit' });
